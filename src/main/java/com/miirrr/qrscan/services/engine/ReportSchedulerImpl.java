...
public class ReportSchedulerImpl implements ReportScheduler{
     ...
     * range from 0 to 59. For hours 0 to 23, for days of the month 0 to 31, and for months 0 to 11 (JAN to DEC).
     * The "/" character simply helps you turn on every "nth" value in the given set. Thus "7/6" in the month field
     * only turns on month "7", it does NOT mean every 6th month, please note that subtlety.
     *
     ...
     * "the last friday of the month". You can also specify an offset from the last day of the month, such as "L-3"
     * which would mean the third-to-last day of the calendar month. When using the 'L' option, it is important not
     ...
     * the 16th. If the 15th is a Tuesday, then it will fire on Tuesday the 15th. However if you specify "1W" as the
     * value for day-of-month, and the 1st is a Saturday, the trigger will fire on Monday the 3rd, as it will not
     * 'jump' over the boundary of a month's days. The 'W' character can only be specified when the day-of-month is
     ...
     * day-of-week in the month, then no firing will occur that month. If the '#' character is used, there can only
     * be one expression in the day-of-week field ("3#1,6#3" is not valid, since there are two expressions).
     */
